...
const deck = generateDeck(["black", "blue", "green", "grey", "orange", "purple", "red", "yellow", "purple"])
let current = 0
...
<template>
  ...
  <div id="card">
    <table>
      ...
      <tbody>
        <tr v-for="row in deck[current]">
          <td><img :src="'./assets/cars/' + row.color + '_car.png'"></td>
          <td>{{ row.cornerSpeed }}</td>
          <td>{{ row.straightSpeed }}</td>
        </tr>
      </tbody>
      <div class="card_footer"><span id="card_counter">{{ current + " / 10" }}</span><button id="button_next" @:click="nextCard()">Next card</button></div>
    </table>
  </div>
</template>
